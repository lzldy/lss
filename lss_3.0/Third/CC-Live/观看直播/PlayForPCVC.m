...
-(NSMutableArray *)publishIdArray {
    if(_publishIdArray==nil || [_publishIdArray count] == 0) {
        _publishIdArray = [[NSMutableArray alloc]init];
    }
    return _publishIdArray;
}

-(void)publish_question:(NSString *)publishId {
    if(![self.publishIdArray containsObject:publishId]) {
...
-(void)dealPPTViewDoubleTap {
    ...
    if (!self.isScreenLandScape) {
        ...
        [UIView animateWithDuration:0.25f animations:^{
            ...
            [ws.requestData setDocFrame:rect];
...
//    NSLog(@"收到提问: questionDic = %@",questionDic);
    ...
    if (questionDic) {
        ...
        if([dialog.fromuserid isEqualToString:dialog.myViwerId] && ![self.publishIdArray containsObject:dialog.encryptId]) {
            [self.publishIdArray addObject:dialog.encryptId];
        }
...
//    NSLog(@"收到回答: answerDic = %@",answerDic);
    ...
    if (answerDic) {
        ...
        if (isPrivate == 0 || (isPrivate == 1 && [questionUserId isEqualToString:self.viewerId])) {
            ...
            if(isPrivate == 0 && ![self.publishIdArray containsObject:dialog.encryptId]) {
                [self.publishIdArray addObject:dialog.encryptId];
            }
...
//    NSLog(@"questionArr = %@,answerArr = %@",questionArr,answerArr);
    ...
    [self.publishIdArray removeAllObjects];
    ...
    for (NSDictionary *dic in questionArr) {
        ...
        if([dic[@"isPublish"] integerValue] == 1 && ![self.publishIdArray containsObject:dialog.encryptId]) {
            [self.publishIdArray addObject:dialog.encryptId];
        }
    ...
    for (NSDictionary *dic in answerArr) {
        ...
        if (isPrivate == 0) {
            ...
            if(![self.publishIdArray containsObject:dialog.encryptId]) {
                [self.publishIdArray addObject:dialog.encryptId];
            }
        }
    }
