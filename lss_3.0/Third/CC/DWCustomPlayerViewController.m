...
- (id)initWithNibName:(NSString *)nibNameOrNil bundle:(NSBundle *)nibBundleOrNil
{
    ...
    if (self) {
       // _qualityDescription = @[@"普通", @"清晰", @"高清"];
        ...
        self.playerView =[[DWPlayerView alloc]initWithUserId:CC_USERID key:CC_APIKEY];
        self.playerView.frame =CGRectMake(0,0,LSMainScreenW,LSMainScreenH/2-50);
...
- (void)btnSkipTime{
    ...
    if (_adInfo.canSkip ){
        ...
        if (_skipTime <= 0) {
            ...
            closeBtn.frame =CGRectMake(LSMainScreenW-80, 20, 70, 30);
            
            _timeLabel.frame =CGRectMake(LSMainScreenW - 117, 20, 30, 30);
            closeBtn.hidden =NO;
            _isClose =YES;
...
{
    _adInfo = [[DWAdInfo alloc]initWithUserId:CC_USERID andVideoId:self.videoId type:_type];
    [_adInfo start];
...
- (void)loadHeaderView
{
    ...
    LsLog(@"headerView frame: %@", NSStringFromCGRect(self.headerView.frame));
...
- (BOOL)loadMovieSubtitle
{
    ...
    if (![self.mediaSubtitle parse]) {
        LsLog(@"path parse failed: %@", [self.mediaSubtitle.error localizedDescription]);
        return NO;
    }
    if (self.isFullscreen) {
...
- (void)loadFooterView
{
    ...
    LsLog(@"footerView: %@", NSStringFromCGRect(self.footerView.frame));
...
-(void)loadSwitchScrBtn
{
    ...
    LsLog(@"self.switchScrBtn.frame: %@", NSStringFromCGRect(self.switchScrBtn.frame));
...
//    [self.movieSubtitleLabel removeFromSuperview];
    ...
    //根据自身视图情况 处理
    CGSize size =CGSizeMake(LSMainScreenH, LSMainScreenW);
...
- (void)loadQualityView
{
    ...
    frame.origin.x =LSMainScreenW-150;
    frame.origin.y = self.playbackButton.frame.origin.y;
    frame.size.width = 50;
    frame.size.height = 30;
...
- (void)switchQuality:(NSInteger)index
{
    ...
    LsLog(@"index: %ld %ld", (long)index, (long)currentQualityIndex);
//    if (index == currentQualityIndex) {
//        //不需要切换
//        logdebug(@"current quality: %ld %@", (long)currentQualityIndex, self.currentPlayUrl);
//        return;
//    }
    LsLog(@"switch %@ -> %@", self.currentPlayUrl, [[self.playUrls objectForKey:@"qualities"] objectAtIndex:index]);
...
{//视频当前播放时间
    ...
    LsLog(@"currentPlaybackTimeLabel frame: %@", NSStringFromCGRect(self.currentPlaybackTimeLabel.frame));
}
...
- (void)loadPlaybackSlider
{
    ...
    LsLog(@"self.durationSlider.frame: %@", NSStringFromCGRect(self.durationSlider.frame));
...
- (void)loadVoiceButton{
       ...
       if (_voiceSlider) {
        ...
        _voiceSlider =[[UISlider alloc]initWithFrame:CGRectMake(LSMainScreenW-100, LSMainScreenH-160, 130, 8)];
...
- (void)playLocalVideo
{
   ...
   //读取Docunment下文件路径 拼接完整的文件路径
    ...
    [self.playerView setURL:[NSURL fileURLWithPath:videoLocalPath] withCustomId:nil];
